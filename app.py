...
def create_target(df, pips=0.15):
    # 未来を見て、+0.15(Up)に先に着くか、-0.15(Down)に先に着くかを判定
    # 1: Up, 0: Down (到達しなかった場合は学習から除外するためにNaN)
    
    targets = []
    # 処理速度向上のため、直近1000件のみ計算
    ...
    for i in range(len(df)):
        ...
        for j in range(i + 1, min(len(df), i + 48)):
            ...
            if hit_up and not hit_down:
                ...
                # 同じ足で両方タッチ(乱高下)。Closeに近い方判定などあるが、今回は除外か、方向維持
                future_result = np.nan 
                break
    ...
    with st.spinner('AIがチャート分析中...'):
        ...
        if df is not None:
            ...
            if len(train_df) > 50: # データが十分ある場合のみ学習
                ...
                # --- 最新の確定足を取得 ---
                # 現在時刻が13:07なら、13:05からの足は「Live」なので未確定。
                # 13:00-13:05の足(indexは13:00)を使う。
                # yfinanceの最後尾がLiveかどうか判定
                last_timestamp = df.index[-1].replace(tzinfo=pytz.utc).astimezone(jst)
                
                # 最新行を使うか、1つ前を使うか
                # 簡易的に「最後から2番目」を確定足とする（yfinanceは直近が動いているため）
                target_row_idx = -2
                ...
                # 過去60本分のデータを取得して予測＆結果判定
                sim_df = df.iloc[-62:-2].copy() # 直近を除いた過去データ
                if not sim_df.empty:
                    ...
                    for i in range(len(sim_df)):
                        ...
                        actual_target = sim_df['Target'].iloc[i] # 1=Up勝ち, 0=Down勝ち, NaN=ドロー
                        
                        result_pips = 0
                        
                        # トレードロジック
                        if p_up >= threshold: # AIがUP予測
                            if actual_target == 1: result_pips = 15
                            elif actual_target == 0: result_pips = -15
                        elif p_up <= (100 - threshold): # AIがDOWN予測 (27%以下)
                            if actual_target == 0: result_pips = 15 # Down的中
                    ...
                    fig.update_layout(
                        ...
                        height=200,
                        showlegend=False,
                        xaxis=dict(showgrid=False, visible=False), # X軸は非表示ですっきり
